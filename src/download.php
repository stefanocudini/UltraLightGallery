...
{
  ?>
  <fieldset class="gruppomenu"><legend>Download</legend>
  <a target="_blank" href="<?=$_SERVER['PHP_SELF']?>?download<? if(isset($_GET['local'])) echo "&amp;local=$local"; ?>"><span>Download all</span></a>
  </fieldset>
  <?
} # */
...
function download_init()
{
	...
	$images['downicon']['dir'] = $dirs['cache_base'].'_downicon.png';
	$images['downicon']['url'] = $urls['cache_base'].'_downicon.png';
	$masks['fotodownload'] = './?download=%s';

	if($admin and isset($_GET['download']))
	  $start = 'download';
}
...
function download_start()
{
	...
	$fotofile = trim($_GET['download']);

	if($admin and is_file(getcript_filename($fotofile)) )
		forceDownload($fotofile); 
}

function download_thumb_menu($fotofile)
{
  global $masks;
  global $images;
  global $admin;
  ...
  if(is_dir($fotofile) or !$admin) return false;
  
  $href = $urls['current'].sprintf($masks['fotodownload'],$fotofile);
  
?><a class="icon down" href="#" title="Scarica foto originale"><span>Download</span><img src="<?=$images['downicon']['url']?>" /></a><?

  return 'Scarica la foto originale';
}

function forceDownload($fotofile)
{  
...
#  header("Content-type: application/octet-stream");  //per tutti i files
  header("Content-Disposition: attachment; filename=".($fotofile).";" );
  header("Content-Transfer-Encoding: binary");
  header("Content-Length: ".@filesize(getcript_filename($fotofile)));
  readfile(getcript_filename($fotofile));
  exit(0);
}
...
function download_cache()
{
  ...
  require('download.cache.php');

  put_contents($images['downicon']['dir'],$icon1);
}
...
add_thumb_event(function(obj) {
    ...
    $(".icon.down", obj).click(function() {
		  var fotofile = $(obj).attr('id');
      location.href = '<?=sprintf($masks['fotodownload'],'')?>' + fotofile;
			return false;
    });
